...
void FileDecoder::run() {
    ...
    while (file.read(payload,512) > 0){
        int idx = 0;
        short count =
                short(((unsigned char)payload[idx + 1] << 8) |
        ...
        for(int i = 1; i <= count; ++i){
            ...
            idx += 4;
            auto accx = short(((unsigned char)payload[idx+1] << 8) |
                         ((unsigned char)payload[idx]));
            idx += 2;
            auto accy = short(((unsigned char)payload[idx + 1] << 8) |
                               ((unsigned char)payload[idx ]));
            idx += 2;
            auto accz = short(((unsigned char)payload[idx + 1] << 8) |
                               ((unsigned char)payload[idx]));
            idx += 2;
            auto gyrox = short(((unsigned char)payload[idx + 1] << 8) |
                               ((unsigned char)payload[idx ]));
            idx += 2;
            auto gyroy = short(((unsigned char)payload[idx + 1] << 8) |
                               ((unsigned char)payload[idx + 0]));
            idx += 2;
            auto gyroz = short(((unsigned char)payload[idx + 1] << 8) |
                               ((unsigned char)payload[idx + 0]));
            idx += 2;
            auto temp = short(((unsigned char)payload[idx + 1] << 8) |
                                ((unsigned char)payload[idx + 0]));
            idx += 2;
            if(idx >= 512) {
